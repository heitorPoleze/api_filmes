...
export class Avaliacao {
    ...
    private _obraId: string; // Corrigido para string
    private _usuarioId: string;

    constructor(nota: number, obraId: string, usuarioId: string, comentario?: string, id?: string) {
    ...
    get obraId(): string { return this._obraId; } // Corrigido para string
    ...
    private verificaNota(nota: number){
        if (!Number.isInteger(nota)){
            throw new Error ("A nota da avaliação deve ser um número inteiro.");
        }
        if(nota < 0 || nota > 10){
    ...
    static fromJson(json: any): Avaliacao {
        ...
        const obraId = String(json.obraId);
