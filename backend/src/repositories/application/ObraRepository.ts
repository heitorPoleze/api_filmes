...
export class ObraRepository {
    ...
    async buscarPorNome(nome: string): Promise<Obra | null> {
        try {
            const obra = await ObraModel.findOne({
                 name: nome
                }).collation({ locale: "pt", strength: 1 }); //forma de permitir case insensitive sem buscar apenas a fracao da busca
                // pt é regra de portugues, como acentuação, e strength 1 permite que a query aceite a falta de acentuação 
            if (!obra) return null;
