...
async atualizar(id: string, obra: ObraDoc): Promise<Obra> {
    ...
    async atualizarNota(idObra: string, novaNotaMedia: number): Promise<Obra> {
        if(!Types.ObjectId.isValid(idObra)) throw new Error("ID da obra inválido para atualização.");
        if(novaNotaMedia < 0 || novaNotaMedia > 10) throw new Error("Nota inválida para atualização.");
        
        const obraAtualizada = await this._obraRepo.atualizarNota(idObra, novaNotaMedia);
