...
import { UsuarioRepository } from "../../repositories/application/UsuarioRepository.ts";
...
export class UsuarioServices {
    ...
    async buscarPorFiltros(nome?: string, email?: string): Promise<Usuario[]>{
        if(nome && nome.length < 3) throw new Error("Nome deve conter no mínimo 3 caracteres");
        if(email && !email.includes("@")) throw new Error("Email inválido");

        return await this._userRepo.buscarPorFiltros(nome, email);
    }
}